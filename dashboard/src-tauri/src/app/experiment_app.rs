...
use crate::database::data::experiment::{
    delete_experiment, experiment_exists, experiment_name_exists, list_experiments,
    store_experiment,
};
...
pub fn experiment_list(
    ...
    let mut app = state
        .lock()
        .map_err(|_| "State lock poisoned".to_string())?;
    list_experiments(app.db_connection_mut()?).map_err(|err| err.to_string())
}
...
pub fn experiment_delete(
    ...
    delete_experiment(app.db_connection_mut()?, experiment_id).map_err(|err| err.to_string())?;
    if app.active_experiment_id == Some(experiment_id) {
