...
<template>
  <div class="flex flex-col items-center space-y-1">
    <template v-if="readOnly">
      <Button
        variant="ghost"
        class="h-12 w-12 rounded-full flex items-center justify-center text-sm"
        :style="{
    ...
    <template v-else>
      <ContextMenu
        ...
        @update:open="handleContextMenuToggle"
      >
        <ContextMenuTrigger>
          <Button
            variant="ghost"
            ...
            @click="handleToggle"
            @wheel.prevent="handleWheelAdjust"
          >
            {{ cell.value }}
          </Button>
        </ContextMenuTrigger>
        <ContextMenuContent class="w-64">
          ...
          <div class="p-3 space-y-3">
            ...
            <WavelengthSlider
              :model-value="cell.wavelength"
              @update:model-value="handleWavelengthChange"
...
const getTextColor = (): string => {
  if (!props.cell.active) return '#ffffff'

  if (props.cell.value < 50) return '#000000'
  else return '#ffffff'
}
